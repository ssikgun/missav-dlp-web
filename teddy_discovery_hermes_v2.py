...
HERMES_V2_SYSTEM_INSTRUCTION: Final[str] = (
    ...
    "Return JSON only. The top-level JSON object must contain exactly one field "
    "named cues. The cues field must be an array containing exactly one output "
    "object for every input cue, in the original request order. Do not use "
    "results or any other top-level field name. Each cue output object must "
    "contain exactly cue_id, repaired_ja, and ko. Do not output timestamps. "
    "Do not output "

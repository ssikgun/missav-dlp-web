...
replace_once(
    ...
    '''            <form id="downloadForm" class="url-bar">
            ...
            <div class="teddy-route-hint" id="teddyRouteHint">자동 · 새 사이트는 Direct 우선 → 실패 시 무료 Proxy → 실패 시 VPN</div>''',
...
replace_once(
    ...
    '''        const meta = card.querySelector('.task-meta');
        const routeLabel = task.network_mode === 'vpn' ? 'VPN' : (task.network_mode === 'proxy' ? 'Proxy' : (task.network_mode === 'direct' ? 'Direct' : ''));
        ...
        const proxyDetail = task.network_mode === 'proxy' && task.network_proxy_latency_ms
            ...
            : '';
        const nextMeta = routeLabel ? statusText + ' · ' + routeLabel + proxyDetail : statusText;
        if (meta.textContent !== nextMeta) meta.textContent = nextMeta;''',
    'task route label',
)
...
replace_once(
    ...
    '''                <div class="setting-group teddy-routing-settings">
                    ...
                    <div class="setting-desc">자동 모드는 저장된 성공 경로를 사용하고, 처음 보는 사이트는 Direct → 무료 Proxy → VPN 순으로 네트워크성 실패에만 다음 경로를 시도합니다. 성공한 경로는 자동 학습됩니다.</div>

                    <div class="teddy-routing-summary">
                        <strong>기본 정책</strong>
                        <span>Direct 우선 → 무료 Proxy → VPN → 성공 경로 학습</span>
                    </div>

                    <div id="teddyProxyPoolMount"></div>

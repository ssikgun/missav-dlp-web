...
RUN python -c "import teddy_hls_transport as h; assert h.HLS_WORKERS == 8; assert h.ALLOWED_HLS_WORKERS == (2, 4, 8, 12, 16); assert h.workers_from_settings({}) == 8; assert h.workers_from_settings({'hls_workers': 2}) == 2; assert h.workers_from_settings({'hls_workers': 4}) == 4; assert h.workers_from_settings({'hls_workers': 8}) == 8; assert h.workers_from_settings({'hls_workers': 12}) == 12; assert h.workers_from_settings({'hls_workers': 16}) == 16; assert h.workers_from_settings({'hls_workers': 3}) == 8; assert callable(h.get); assert callable(h.invalidate); print('persistent HLS transport + worker benchmark smoke test: OK')"
